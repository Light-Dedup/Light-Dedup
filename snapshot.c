	...
	u64 new_block)
{
	...
	for (i = 0; i < list->num_pages; i++) {
		/* Leave next_page field alone */
		nova_memunlock_block(sb, curr_nvmm_addr, &flags);
		memcpy_to_pmem_nocache(curr_nvmm_addr, (void *)curr_dram_addr,
						LOG_BLOCK_TAIL);
		nova_memlock_block(sb, curr_nvmm_addr, &flags);
