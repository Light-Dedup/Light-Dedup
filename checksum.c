...
static int nova_repair_entry_pr(struct super_block *sb, void *entry)
{
	...
	nova_memunlock_range(sb, entry_pr, POISON_RADIUS, &flags);
	ret = memcpy_mcsafe(entry_pr, alter_pr, POISON_RADIUS);
	nova_memlock_range(sb, entry_pr, POISON_RADIUS, &flags);
	...
	size_t entry_size)
{
	...
	nova_memunlock_range(sb, bad, entry_size, &flags);
	ret = memcpy_to_pmem_nocache(bad, good, entry_size);
	nova_memlock_range(sb, bad, entry_size, &flags);
	...
	struct nova_inode *bad_pi, struct nova_inode *good_pi)
{
	...
	nova_memunlock_range(sb, bad_pr, POISON_RADIUS, &flags);
	ret = memcpy_mcsafe(bad_pr, good_pr, POISON_RADIUS);
	nova_memlock_range(sb, bad_pr, POISON_RADIUS, &flags);
	...
	struct nova_inode *good_copy)
{
	...
	nova_memunlock_inode(sb, bad_pi, &flags);
	ret = memcpy_to_pmem_nocache(bad_pi, good_copy,
					sizeof(struct nova_inode));
	nova_memlock_inode(sb, bad_pi, &flags);
...
copy:
		...
		nova_memunlock_range(sb, csum_addr, NOVA_DATA_CSUM_LEN * 8, &flags);
		if (support_clwb) {
		...
		nova_memlock_range(sb, csum_addr, NOVA_DATA_CSUM_LEN * 8, &flags);
		if (support_clwb) {
	...
	for (strp = 0; strp < strps; strp++) {
		...
		nova_memunlock_range(sb, csum_addr, NOVA_DATA_CSUM_LEN, &flags);
		memcpy_to_pmem_nocache(csum_addr, &csum, NOVA_DATA_CSUM_LEN);
		memcpy_to_pmem_nocache(csum_addr1, &csum, NOVA_DATA_CSUM_LEN);
		nova_memlock_range(sb, csum_addr, NOVA_DATA_CSUM_LEN, &flags);
	...
	size_t offset, size_t bytes)
{
	...
	for (strp = 0; strp < strps; strp++) {
		...
		if (csum_nvmm0 != csum_nvmm1) {
			...
			nova_memlock_range(sb, csum_addr0, NOVA_DATA_CSUM_LEN, &flags);
