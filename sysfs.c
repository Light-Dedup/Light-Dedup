...
static int nova_seq_IO_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "hints of fpentry: no hint %llu, "
		...
		IOstats[no_hint], IOstats[hint_not_trusted_hit],
		IOstats[hint_not_trusted_miss]);
	seq_printf(seq, "Prefetch next: %llu, hit: %llu\n",
		IOstats[prefetch_next], IOstats[prefetch_hit]);
